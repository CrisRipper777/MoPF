#!/usr/bin/env python3
"""Render the final empirical-motivation Figure 1 (v4).
...
def _panel_c_stacked(ax: Any, data_dir: Path) -> None:
    ...
    hop_grays = ("#E6E6E6", "#BDBDBD", "#969696", "#5A5A5A")
    hop_handles = [Patch(facecolor=color, edgecolor=NEUTRAL, linewidth=0.35,
                         label=str(hop)) for hop, color in enumerate(hop_grays)]
    modality_handles = [
    ...
    # Reserve a small blank strip above the 100% bars for compact legends;
    ...
    modality_legend = ax.legend(handles=modality_handles, loc="upper left",
                                bbox_to_anchor=(0.01, 0.97), ncol=2,
    ...
    ax.legend(handles=hop_handles, title="Hop order", loc="upper right",
              bbox_to_anchor=(1.0, 0.97), ncol=4, borderaxespad=0.0,
              handlelength=0.9, handleheight=0.8, handletextpad=0.3,
              columnspacing=0.55, labelspacing=0.2, borderpad=0.15,
              title_fontsize=7.0)
    _format_axis(ax)
...
def _alignment_gate(fig: Any, axes: list[Any], output_dir: Path) -> None:
    ...
    report = require_matplotlib_panel_alignment(
        fig,
        json_out=output_dir / "figure1_empirical_final_v4.alignment.json",
        overlay_svg=output_dir / "figure1_empirical_final_v4.alignment.svg",
        tolerance_pt=1.5,
    ...
    print(f"[figure1-v4] panel alignment: {report.get('verdict', 'checked')}", flush=True)
...
def main() -> None:
    ...
    _standalone(_panel_a, data_dir, output_dir,
                "figure1a_relation_discrepancy_final_v4", "a", 96.0, 78.0)
    _standalone(_panel_b, data_dir, output_dir,
                "figure1b_propagation_drift_final_v4", "b", 96.0, 78.0)
    _standalone(_panel_c_stacked, data_dir, output_dir,
                "figure1c_order_heterogeneity_final_v4", "c", 112.0, 78.0)
    ...
    fig.subplots_adjust(left=0.075, right=0.99, bottom=0.24, top=0.80, wspace=0.47)
    ...
    _save_formats(fig, output_dir, "figure1_empirical_final_v4")

    manifest = {
        "figure": "Figure 1. Empirical observations behind CoSI-MAG (v4)",
    ...
    (output_dir / "figure1_empirical_final_v4_manifest.json").write_text(
        json.dumps(manifest, indent=2, ensure_ascii=False) + "\n", encoding="utf-8"
    )
    print(f"[figure1-v4] wrote v4 panels and combined figure to {output_dir}", flush=True)
