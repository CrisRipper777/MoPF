#!/usr/bin/env python3
"""Render the final empirical-motivation Figure 1 (v2).
...
DATASET_LABELS = ("Movies", "Toys", "Grocery", "ele-\nfashion", "Reddit-S")
...
def _panel_a(ax: Any, data_dir: Path) -> None:
    ...
    for yi, (q10, q25, median, q75, q90) in zip(y, stats):
        ax.hlines(yi, q10, q90, color=DISCREPANCY, linewidth=0.9, alpha=0.46, zorder=2)
        ax.hlines(yi, q25, q75, color=DISCREPANCY, linewidth=4.2, alpha=0.95, zorder=3)
        ax.scatter(median, yi, s=28, color=DISCREPANCY, edgecolor="white",
    ...
    ax.set_title("Relation discrepancy", loc="left", pad=7, fontweight="bold")
...
def _panel_b(ax: Any, data_dir: Path) -> None:
    ...
    ax.set_ylabel("Representation drift from H0 (%)", labelpad=4)
    ax.set_title("Propagation drift", loc="left", pad=7, fontweight="bold")
...
def _panel_c_stacked(ax: Any, data_dir: Path) -> None:
    ...
    colors = {"text": TEXT_BLUE, "visual": VISUAL_GREEN}
    alpha = (0.38, 0.52, 0.66, 0.80)
    for modality in MODALITIES:
        bottom = np.zeros(len(DATASETS), dtype=np.float64)
        for hop, opacity in enumerate(alpha):
            heights = np.asarray([
                fractions[(dataset, modality, hop)] for dataset in DATASETS
            ], dtype=np.float64)
            ax.bar(x + offsets[modality], heights, width=width, bottom=bottom,
                   color=colors[modality], alpha=opacity, edgecolor="white",
                   linewidth=0.45, zorder=3)
            bottom += heights

    ax.legend(handles=[
        Patch(facecolor=TEXT_BLUE, edgecolor=TEXT_BLUE, alpha=0.65, label="Text"),
        Patch(facecolor=VISUAL_GREEN, edgecolor=VISUAL_GREEN, alpha=0.65, label="Visual"),
        *[
            Patch(facecolor=NEUTRAL, edgecolor=NEUTRAL, alpha=alpha[hop], label=f"k={hop}")
            for hop in HOPS
        ],
    ], loc="lower center", bbox_to_anchor=(0.5, 1.02), ncol=6,
       borderaxespad=0.0, handletextpad=0.35, columnspacing=0.65)
    ax.set_xticks(x, labels=DATASET_LABELS)
    ...
    # The compact alternative uses the panel label and the two-part legend as
    # its heading, leaving the top margin free of a second title line.
    _format_axis(ax)
...
def _alignment_gate(fig: Any, axes: list[Any], output_dir: Path) -> None:
    ...
    report = require_matplotlib_panel_alignment(
        fig,
        json_out=output_dir / "figure1_empirical_final_v2.alignment.json",
        overlay_svg=output_dir / "figure1_empirical_final_v2.alignment.svg",
        tolerance_pt=1.5,
    ...
    print(f"[figure1-v2] panel alignment: {report.get('verdict', 'checked')}", flush=True)
...
def main() -> None:
    ...
    _standalone(_panel_a, data_dir, output_dir,
                "figure1a_relation_discrepancy_final_v2", "a", 96.0, 78.0)
    _standalone(_panel_b, data_dir, output_dir,
                "figure1b_propagation_drift_final_v2", "b", 96.0, 78.0)
    _standalone(_panel_c_violin, data_dir, output_dir,
                "figure1c_order_heterogeneity_violin", "c", 112.0, 78.0)
    _standalone(_panel_c_stacked, data_dir, output_dir,
                "figure1c_order_heterogeneity_stacked", "c", 112.0, 78.0, top=0.68)
    ...
    fig.subplots_adjust(left=0.075, right=0.99, bottom=0.20, top=0.80, wspace=0.47)
    _panel_a(axes[0], data_dir)
    _panel_b(axes[1], data_dir)
    _panel_c_violin(axes[2], data_dir)
    for axis, label in zip(axes, "abc"):
        _add_panel_label(axis, label)
    _alignment_gate(fig, list(axes), output_dir)
    _save_formats(fig, output_dir, "figure1_empirical_final_v2")

    manifest = {
        "figure": "Figure 1. Empirical observations behind CoSI-MAG (v2)",
        "panel_a": "Per-dataset distribution of 100*abs(text edge-rank - visual edge-rank), shown as median/IQR and 10-90% whisker",
        "panel_b": "Across-dataset mean of 100*(1-cosine(H0, S_k)) with dataset-level IQR ribbon",
        "panel_c": "Model-independent node-level best-order choices from identical per-hop linear probes",
        "panel_c_probe_datasets": list(DATASETS),
        "panel_c_default": "violin",
        "panel_c_alternative": "100% stacked best-order fractions",
        "panel_c_node_source": "order_utility_node_choices.csv",
    ...
    (output_dir / "figure1_empirical_final_v2_manifest.json").write_text(
        json.dumps(manifest, indent=2, ensure_ascii=False) + "\n", encoding="utf-8"
    )
    print(f"[figure1-v2] wrote v2 panels and combined figure to {output_dir}", flush=True)
