#!/usr/bin/env python3
"""Render the final empirical-motivation Figure 1 (v3).
...
VISUAL_GREEN = "#59A14F"
DISCREPANCY = "#D17C4B"
RELATION_MEDIAN = "#356A9A"
RELATION_RANGE = "#B9C9D8"
NEUTRAL = "#8A8F93"
...
def _panel_a(ax: Any, data_dir: Path) -> None:
    ...
    for yi, (q10, q25, median, q75, q90) in zip(y, stats):
        ...
        ax.scatter(median, yi, s=28, color=RELATION_MEDIAN, edgecolor="white",
    ...
    ax.set_xlabel("Cross-modal edge-rank gap", labelpad=4)
...
def _panel_c_stacked(ax: Any, data_dir: Path) -> None:
    ...
    # A compact note replaces the mixed modality/order legend.
    ax.text(0.02, 0.99, "Text: blue shades", transform=ax.transAxes,
            ha="left", va="top", color=TEXT_BLUE, fontweight="bold")
    ax.text(0.02, 0.93, "Visual: green shades", transform=ax.transAxes,
            ha="left", va="top", color=VISUAL_GREEN, fontweight="bold")
    ax.text(0.02, 0.87, "Lighter → lower hop order;",
            transform=ax.transAxes, ha="left", va="top", color=NEUTRAL,
            fontsize=7.0)
    ax.text(0.02, 0.81, "darker → higher hop order",
            transform=ax.transAxes, ha="left", va="top", color=NEUTRAL,
            fontsize=7.0)
    ax.set_xticks(x, labels=DATASET_LABELS)
    ...
    ax.set_title("Preferred hop-order\nheterogeneity", loc="left", pad=7, fontweight="bold")
    _format_axis(ax)
...
def _alignment_gate(fig: Any, axes: list[Any], output_dir: Path) -> None:
    ...
    report = require_matplotlib_panel_alignment(
        fig,
        json_out=output_dir / "figure1_empirical_final_v3.alignment.json",
        overlay_svg=output_dir / "figure1_empirical_final_v3.alignment.svg",
        tolerance_pt=1.5,
    ...
    print(f"[figure1-v3] panel alignment: {report.get('verdict', 'checked')}", flush=True)
...
def main() -> None:
    ...
    _standalone(_panel_a, data_dir, output_dir,
                "figure1a_relation_discrepancy_final_v3", "a", 96.0, 78.0)
    _standalone(_panel_b, data_dir, output_dir,
                "figure1b_propagation_drift_final_v3", "b", 96.0, 78.0)
    _standalone(_panel_c_stacked, data_dir, output_dir,
                "figure1c_order_heterogeneity_final_v3", "c", 112.0, 78.0)
    ...
    _save_formats(fig, output_dir, "figure1_empirical_final_v3")

    manifest = {
        "figure": "Figure 1. Empirical observations behind CoSI-MAG (v3)",
        "panel_a": "Per-dataset distribution of 100*abs(text edge-rank - visual edge-rank), shown as blue median/IQR and 10-90% whisker",
    ...
    (output_dir / "figure1_empirical_final_v3_manifest.json").write_text(
        json.dumps(manifest, indent=2, ensure_ascii=False) + "\n", encoding="utf-8"
    )
    print(f"[figure1-v3] wrote v3 panels and combined figure to {output_dir}", flush=True)
